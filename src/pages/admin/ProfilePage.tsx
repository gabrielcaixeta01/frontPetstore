  ...
  return (
    <div className="mx-auto w-full max-w-4xl px-4 py-6 sm:px-6 sm:py-8">
      <div className="space-y-5">
        ...
        <div className="relative overflow-hidden rounded-md bg-[#1c46f3] px-6 py-8 sm:px-8">
          <div className="relative z-10 flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
            <div className="flex items-end gap-4">
              ...
              <div>
                <p className="text-[10px] font-semibold uppercase tracking-wider text-white/55">Administrador</p>
                <h1 className="text-2xl font-extrabold leading-tight text-white">{profile.name}</h1>
                <div className="mt-1.5 flex flex-wrap items-center gap-2">
                  {lojaName && <span className="flex items-center gap-1 text-xs text-white/70"><Building2 size={11} /> {lojaName}</span>}
                  {ep?.cargo && <span className="rounded-full border border-white/25 bg-white/15 px-2 py-0.5 text-[11px] text-white">{ep.cargo}</span>}
            ...
            <div className="flex flex-col items-start gap-3 sm:items-end">
              {!isEditing && (
                <button onClick={() => { setEditName(profile.name); setEditEmail(profile.email); setEditPhone(profile.phone ?? ""); setEditError(""); setIsEditing(true); }}
              ...
              <div className="flex gap-2.5">
                ...
                <div className="rounded border border-white/15 bg-white/10 px-4 py-2.5">
                  ...
                  <p className="mt-0.5 text-base font-extrabold leading-none text-[#F5A800]">{calcTempoCasa(ep?.data_contratacao)}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        {/* ── Dados Pessoais ─────────────────────────────── */}
        <SectionCard iconBg="bg-[#e8eeff]" iconCls="text-[#1c46f3]" IconEl={User} title="Dados Pessoais" sub="Informações cadastrais do administrador">
          {isEditing ? (
        ...
        {lojaName && (
          <SectionCard iconBg="bg-[#e6f4ed]" iconCls="text-[#00A651]" IconEl={Building2} title="Loja Vinculada" sub="Unidade de trabalho">
            <div className="grid divide-gray-100 sm:grid-cols-3 sm:divide-x">
              ...
              <div className="px-5 py-3.5">
                ...
                <p className="mt-1.5">
                  <span className="inline-flex items-center gap-1.5 rounded-full border border-emerald-200 bg-emerald-50 px-2.5 py-0.5 text-[11px] font-semibold text-[#00A651]">
                    <span className="h-1.5 w-1.5 rounded-full bg-[#00A651]" /> Ativo
                  </span>
                </p>
              </div>
        ...
        {recentAtend.length > 0 && (
          <SectionCard iconBg="bg-[#e8eeff]" iconCls="text-[#1c46f3]" IconEl={CalendarCheck} title="Atividade Recente" sub="Últimos atendimentos realizados">
            <div className="divide-y divide-gray-100">
              {recentAtend.map((at) => {
