...
function calcTempoCasa(dataContratacao?: string): string {
    ...
    async function init() {
      try {
        ...
        if (fullUser?.employee_profile) {
          ...
          const loja = lojaData.find((l) => l.id === fullUser.employee_profile!.loja_id);
          setLojaName(loja?.nome ?? "");
        }

        const myAtend = atendData
          .filter((a) => a.funcionario_id === userData.id)
    ...
    <div className="mx-auto w-full max-w-4xl px-4 py-6 sm:px-6 sm:py-8">
      <div className="space-y-5">
        ...
        <div className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm">
          <div className="bg-gradient-to-r from-[#1c46f3] to-violet-600 px-6 py-6">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-xs font-semibold uppercase tracking-wide text-white/60">Funcionário</p>
                  ...
                  {lojaName && (
                    <p className="mt-0.5 flex items-center gap-1 text-sm text-white/70">
                      <Building2 size={12} /> {lojaName}
                    </p>
                  )}
                  {ep?.cargo && <p className="mt-0.5 text-xs text-white/50">{ep.cargo}</p>}
                </div>
              </div>
              <div className="flex gap-3">
                ...
                <div className="rounded-xl bg-white/15 px-4 py-2.5 text-white">
                  <p className="text-xs font-medium text-white/70">Tempo de casa</p>
                  <div className="flex items-baseline gap-1">
                    <span className="text-lg font-bold leading-tight">{calcTempoCasa(ep?.data_contratacao)}</span>
                    <Clock size={13} className="mb-0.5 opacity-70" />
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="flex items-center justify-between px-6 py-3">
            <div className="flex items-center gap-2">
              ...
              {ep?.matricula && <span className="text-xs text-gray-400">Mat. {ep.matricula}</span>}
            </div>
            {!isEditing && (
              <button
                onClick={startEdit}
        ...
        <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
          ...
          {isEditing ? (
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              ...
              {ep && (
                <>
                  <InfoField label="Cargo" value={ep.cargo} />
                  <InfoField label="Contratado em" value={formatDateLong(ep.data_contratacao)} />
                </>
              )}
            </div>
          )}
          ...
          {showPwdForm && (
            <form onSubmit={handleChangePassword} className="mt-3 space-y-2.5">
              ...
              <div className="relative">
                <input
                  type={showNewPwdVis ? "text" : "password"}
                  placeholder="Nova senha (mín. 6 caracteres)"
                  value={newPwd}
                  onChange={(e) => setNewPwd(e.target.value)}
                  className={inputCls}
                />
                <button type="button" onClick={() => setShowNewPwdVis((v) => !v)}
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
