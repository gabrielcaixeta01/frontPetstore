...
export default function PetsPage() {
  ...
  const speciesCounts = useMemo(() => {
    ...
    pets.forEach((p) => { const k = getSpeciesKey(categoriasById[p.categoria_id] ?? ""); counts[k] = (counts[k] ?? 0) + 1; });
    return counts;
  }, [pets, categoriasById]);

  const filterOptions = useMemo(() => {
    const keys = new Set(pets.map((p) => getSpeciesKey(categoriasById[p.categoria_id] ?? "")));
  ...
  const filteredPets = useMemo(() => {
    ...
    return result;
  }, [pets, speciesFilter, search, categoriasById, donosById]);
    ...
    <div className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 sm:py-8">
      ...
      <div className="relative mb-5 overflow-hidden rounded-md bg-[#1c46f3] px-6 py-7 sm:px-8">
        <div className="mb-2 flex items-center gap-1.5 text-[11px] text-white/60">
          <span>Painel Administrativo</span><ChevronRight size={10} />
        ...
        <div className="mb-1 flex items-center gap-2">
          ...
          <h1 className="text-2xl font-extrabold leading-tight text-white">
            Pets <span className="text-[#F5A800]">Cadastrados</span>
          </h1>
        </div>
        <p className="mb-4 text-[13px] text-white/70">Gerencie todos os pets cadastrados no sistema</p>
        {!loading && (
      ...
      {loading ? (
        ...
        <div className="rounded border border-dashed border-gray-200 bg-white p-14 text-center">
          ...
          <p className="text-sm text-gray-400">{pets.length === 0 ? "Nenhum pet cadastrado no sistema." : "Nenhum pet encontrado."}</p>
          {pets.length === 0 && <button onClick={() => setShowForm(true)} className="mt-2 text-sm font-semibold text-[#1c46f3] hover:underline">Cadastrar primeiro pet</button>}
        </div>
      ) : (
