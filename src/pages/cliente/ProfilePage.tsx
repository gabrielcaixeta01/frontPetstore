  ...
  return (
    <div className="mx-auto w-full max-w-4xl px-4 py-6 sm:px-6 sm:py-8">
      <div className="space-y-5">
        ...
        <div className="relative overflow-hidden rounded-md px-6 py-8 sm:px-8" style={{ background: TEAL }}>
          ...
          <div className="relative z-10 flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
                ...
                <div className="mt-1.5 flex flex-wrap items-center gap-2">
                  {cp?.city && (
                    <span className="flex items-center gap-1 text-xs text-white/70">
                      <MapPin size={11} /> {cp.city}{cp.state ? `, ${cp.state}` : ""}
                    </span>
                  )}
            ...
            <div className="flex flex-col items-start gap-3 sm:items-end">
              {!isEditing && (
                <button
                  onClick={() => { setEditName(profile.name); setEditEmail(profile.email); setEditPhone(profile.phone ?? ""); setEditCep(cp?.cep ?? ""); setEditState(cp?.state ?? ""); setEditCity(cp?.city ?? ""); setEditError(""); setIsEditing(true); }}
              ...
              <div className="flex gap-2.5">
                <div className="rounded border border-white/15 bg-white/10 px-4 py-2.5">
                  ...
                  <p className="mt-0.5 text-xl font-extrabold leading-none" style={{ color: AMBER }}>{petCount}</p>
                </div>
                <div className="rounded border border-white/15 bg-white/10 px-4 py-2.5">
        ...
        {/* ── Dados Pessoais ─────────────────────────────── */}
        <SectionCard iconBg="bg-[#e6f5f5]" iconCls="text-[#0D7377]" IconEl={User} title="Dados Pessoais" sub="Informações cadastrais da conta">
          {isEditing ? (
            <div className="space-y-3 p-5">
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <div>
                  <label className="mb-1 block text-xs font-medium" style={{ color: MUTED }}>CEP</label>
                  <input className={inputCls} value={editCep} onChange={(e) => setEditCep(e.target.value)} />
                </div>
                <div>
                  <label className="mb-1 block text-xs font-medium" style={{ color: MUTED }}>Estado (UF)</label>
                  <input className={inputCls} value={editState} maxLength={2} onChange={(e) => setEditState(e.target.value.toUpperCase().slice(0, 2))} />
                </div>
                <div className="sm:col-span-2">
                  <label className="mb-1 block text-xs font-medium" style={{ color: MUTED }}>Cidade</label>
        ...
        {/* ── Resumo da conta ────────────────────────────── */}
        <SectionCard iconBg="bg-amber-50" iconCls="text-[#F59E0B]" IconEl={PawPrint} title="Resumo da Conta" sub="Visão geral de pets e atendimentos">
        ...
        {recentAtend.length > 0 && (
          <SectionCard iconBg="bg-[#e6f5f5]" iconCls="text-[#0D7377]" IconEl={CalendarCheck} title="Atividade Recente" sub="Seus últimos atendimentos">
            <div className="divide-y divide-gray-100">
              {recentAtend.map((at) => {
